...
**Learning:** during the session, multiple API handlers were missing consistent error handling.

**Proposed:** add `.agent/rules/api-error-handling.md` that documents the standard error shape, a helper for throwing typed errors, and examples for common failure modes.

### Updating an Existing Skill

**Learning:** the `commit` skill kept tripping over missing trailing newlines.

**Proposed:** add an explicit "trailing newline" bullet to the Pre-Commit Hook Awareness section of `.agent/skills/commit/SKILL.md`.

### Automation Opportunity

**Learning:** the team kept forgetting to update docs after feature changes.

**Proposed:** add a CI check (`scripts/check-docs-updated.sh`) that scans the diff for new features and verifies a corresponding file exists in the documentation folder.
...
- `AGENTS.md` should reference new rules from the Quick-Reference TL;DR if they are important enough
- If another agent surface (Cursor, Claude, Copilot) has its own rules folder, ensure it is symlinked to or generated from `.agent/rules/` so edits propagate everywhere
